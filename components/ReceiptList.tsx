...
function ReceiptList() {
    ...
    console.log("receipts from convex!!", receipts)
    if (!user) {
        return (
    ...
    if (!receipts) {
        return (
            <div className="w-full p-8 tex-center">
    ...
    if (receipts.length === 0) {
        return (
            <div className="w-full p-8 text-center border border-gray-200 rounded-lg bg-gray-50">
                <p className="text-gray-600">No receipts have been upload yet.</p>
            </div>
        )
    }
    return (
                    ...
                    <TableBody>
                        {receipts.map((receipt: Doc<"receipts">) => (
                            ...
                            >
                                ...
                                <TableCell>
                                    <span
                                    className={`px-2 py-1 rounded-full text-xs ${receipt.status === "pending" ? "bg-yellow-100 text-yellow-800" : receipt.status === "processed" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-500"}`}
                                    >
